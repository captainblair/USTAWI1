...
class LandlordLeaseDocumentFileView(APIView):
    permission_classes = [IsAuthenticated, IsLandlordAgentOrAdmin, IsLeaseLandlord]

    @extend_schema(tags=["Landlord Leases"], summary="Download lease document PDF")
    def get(self, request, pk, doc_id):
        lease = (
            ...
            .get(pk=pk, landlord=request.user)
        )
        doc = LeaseDocument.objects.get(pk=doc_id, lease=lease)
        try:
...
class LandlordLeaseSignedPdfDownloadView(APIView):
    permission_classes = [IsAuthenticated, IsLandlordAgentOrAdmin, IsLeaseLandlord]

    @extend_schema(tags=["Landlord Leases"], summary="Download signed lease PDF")
    def get(self, request, pk):
        lease = (
            Lease.objects.select_related(
                "property",
